"""Acceptance tests for Turkey Inventory expansion package 1."""
# ruff: noqa: D103
...
def test_package1_registry_alias_inventory_and_routes(
    platform_id: str,
    canonical_name: str,
    alias: str,
    video_id: str,
    ...
    assert index.find_exact_match(canonical_name).platform_id == platform_id  # type: ignore[union-attr]
    assert index.find_exact_match(alias).platform_id == platform_id  # type: ignore[union-attr]
...
async def test_package1_raw_vlm_end_to_end_is_verified_low(
    platform_id: str,
    canonical_name: str,
    _alias: str,
    video_id: str,
...
def test_package1_ui_catalog_is_registry_and_inventory_backed() -> None:
    ...
    assert catalog["PLT_T129_ATAK"]["user_type"] == "Helikopter"
    assert catalog["PLT_T625_GOKBEY"]["user_type"] == "Helikopter"
    assert catalog["PLT_A400M"]["user_type"] == "Nakliye u\u00e7a\u011f\u0131"
    assert all(row["user_type"] not in {"UAV", "UCAV"} for row in catalog.values())


def test_existing_demo_scenario_catalog_remains_unchanged() -> None:
